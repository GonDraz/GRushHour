...
namespace GonDraz.Events
{
    public sealed class GEvent<T1, T2>
    {
        ...
        public void Invoke(T1 parameter1, T2 parameter2, Action onComplete = null)
        {
            ...
            foreach (var d in _action.GetInvocationList()) CallAction((Action<T1, T2>)d, parameter1, parameter2);

            onComplete?.Invoke();
        }

        private void CallAction(Action<T1, T2> action, T1 parameter1, T2 parameter2)
        {
            if (action == null) return;
            try
            {
        ...
        {
            return action == null ? null : new GEvent<T1, T2>(action);
        }
